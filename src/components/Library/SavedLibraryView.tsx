...
export const SavedLibraryView: React.FC = () => {
  ...
  // Only show projects the user has liked
  const savedProjects = publicProjects.filter(p => p.likes?.includes(user?.id || ''));
      ...
      <div className="max-w-6xl mx-auto px-8 py-12 relative z-10">
        <header className="mb-8 text-center">
          <div className="inline-flex items-center justify-center p-3 bg-indigo-500/10 rounded-2xl mb-4 border border-indigo-500/20">
            <Bookmark className="w-8 h-8 text-indigo-400 stroke-[1.5]" />
          </div>
          <h1 className="text-4xl font-extrabold text-white tracking-tight mb-3">Your Library</h1>
          <p className="text-lg text-slate-400 max-w-2xl mx-auto">
            Novels you've saved and loved from the Novelist Workspace community.
          </p>
        </header>
        ...
        <div className="bg-slate-900/60 backdrop-blur-md border border-slate-800 rounded-2xl p-4 mb-10 flex flex-col md:flex-row items-center gap-4 shadow-xl">
          <div className="flex-1 relative w-full">
            ...
            <input 
              type="text" 
              placeholder="Search saved novels..." 
              value={searchQuery}
          ...
          if (filteredProjects.length === 0) {
            return (
              <div className="text-center py-24 bg-slate-900/30 rounded-3xl border border-slate-800/50 backdrop-blur-sm">
                ...
                <h3 className="text-xl font-semibold text-slate-300 mb-2">No Saved Novels</h3>
                <p className="text-slate-500 max-w-md mx-auto">
                  {savedProjects.length === 0 
                    ? "You haven't saved any novels yet. Explore the Public Library to find your next great read!"
                    : "No saved novels match your current search or filter."}
                </p>
              </div>
            );
          }
